...
export async function getCollections(language: string = 'en'): Promise<Collection[]> {
  "use cache";
  cacheTag(TAGS.collections);
  cacheLife("days");
...
export async function getMenu(handle: string, lang: string = 'en'): Promise<Menu[]> {
  "use cache";
  cacheTag(TAGS.collections);
  cacheLife("days");
  ...
  return (
    res.body?.data?.menu?.items.map((item: { title: string; url: string }) => {
      ...
      // Handle "All Products" / "Catalog" link - /search/all should be /search
      if (path === "/search/all") {
        path = "/search";
      }
